...
async def test_full_upgrade_flow(client, db_session):
    ...
    theme = Theme(
        name="Integration Theme", is_active=True, cpc_prefixes=["G06F"], keywords=["test"]
    )
    ...
    r = await client.post(
        ...
        json={"theme_id": str(theme.id), "mode": "instant_alert"},
        cookies=ck,
    )
    ...
    r = await client.post(
        "/api/v1/subscriptions",
        json={"theme_id": str(theme.id), "mode": "weekly_digest"},
        cookies=ck,
    )
    ...
    event = {
        ...
        "data": {
            "object": {
                ...
                "metadata": {"user_id": "upgrade-test", "tier": "basic"},
            }
        },
    }
    ...
    billing = (
        await db_session.execute(
            select(BillingSubscription).where(BillingSubscription.user_id == "upgrade-test")
        )
    ).scalar_one()
    ...
    event2 = {
        ...
        "data": {
            "object": {
                ...
                "metadata": {"user_id": "upgrade-test", "tier": "enterprise"},
            }
        },
    }
    with patch("app.api.v1.billing.verify_webhook_signature", return_value=event2):
        r = await client.post(
            "/api/v1/billing/webhook",
            content=json.dumps(event2).encode(),
            headers={"stripe-signature": "sig"},
        )
        assert r.status_code == 200
