...
async def test_backfill_skips_already_populated(db_session):
    ...
    patent = PatentPublication(
        doc_id="USPTO:bf-skip",
        publication_number="BFSKIP",
        office="USPTO",
        title="Skip",
        assignees=["S"],
        cpc=["G06F"],
...
async def test_backfill_respects_limit(db_session):
    ...
    for i in range(5):
        patent = PatentPublication(
            doc_id=f"USPTO:bf-limit-{i}",
            publication_number=f"BFL{i}",
            office="USPTO",
            title=f"P{i}",
            assignees=["X"],
            cpc=["G06F"],
