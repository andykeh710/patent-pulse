...
async def test_instant_alert_skips_below_min_score(db_session):
    ...
    patent = PatentPublication(
        doc_id="USPTO:skip-alert", title="Skip Alert Patent", office="USPTO",
        publication_number="USA001", assignees=["SkipCo"], cpc=["G06N"],
    ...
    sub = TopicSubscription(user_id="alert-skip-user", theme_id=theme.id, mode="instant_alert", min_score=70)
    db_session.add(sub)
    await db_session.commit()
    await db_session.refresh(sub)

    from app.tasks.send_instant_alert import _send_instant_alert_async
    r = await _send_instant_alert_async(str(sub.id), str(patent.id), str(uuid.uuid4()), session=db_session)
...
async def test_instant_alert_idempotent_within_hour(db_session):
    ...
    patent = PatentPublication(
        doc_id="USPTO:idem-alert", title="Idem Alert Patent", office="USPTO",
        publication_number="USA002", assignees=["IdemCo"], cpc=["G06N"],
        filing_date=date(2024, 1, 1), publication_date=date(2025, 1, 1),
    )
    db_session.add(patent)
    await db_session.commit()
    ...
    sub = TopicSubscription(user_id="alert-idem-user", theme_id=theme.id, mode="instant_alert", min_score=None)
    db_session.add(sub)
    await db_session.commit()
    await db_session.refresh(sub)

    delivery = EmailDelivery(
        user_id="alert-idem-user", subscription_id=sub.id, email_type="instant_alert",
        status="dev", sent_at=datetime.now(timezone.utc),
    )
...
async def test_instant_alert_sends_email(db_session):
    ...
    patent = PatentPublication(
        doc_id="USPTO:happy-alert", title="Happy Alert Patent", office="USPTO",
        publication_number="USA003", assignees=["HappyCo"], cpc=["G06N"],
        filing_date=date(2024, 1, 1), publication_date=date(2025, 1, 1),
    )
    db_session.add(patent)
    await db_session.commit()
    ...
    sub = TopicSubscription(user_id="alert-happy-user", theme_id=theme.id, mode="instant_alert", min_score=None)
    db_session.add(sub)
    await db_session.commit()
    await db_session.refresh(sub)
...
async def test_hook_enqueues_per_subscription(db_session):
    ...
    patent = PatentPublication(
        doc_id="USPTO:hook-alert", title="Hook Alert Patent", office="USPTO",
        publication_number="USA004", assignees=["HookCo"], cpc=["G06N"],
        filing_date=date(2024, 1, 1), publication_date=date(2025, 1, 1),
    )
    db_session.add(patent)
    await db_session.commit()
