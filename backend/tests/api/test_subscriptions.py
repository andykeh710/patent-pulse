...
async def test_create_subscription(client, db_session):
    ...
    r = await client.post(
        ...
        json={"theme_id": tid, "mode": "weekly_digest"},
...
async def test_duplicate_subscription_returns_409(client, db_session):
    ...
    r1 = await client.post(
        "/api/v1/subscriptions", json={"theme_id": tid, "mode": "instant_alert"}, cookies=cookie
    )
...
async def test_patch_own_subscription(client, db_session):
    ...
    r1 = await client.post(
        "/api/v1/subscriptions", json={"theme_id": tid, "mode": "weekly_digest"}, cookies=cookie
    )
    sub_id = r1.json()["id"]

    r2 = await client.patch(
        f"/api/v1/subscriptions/{sub_id}", json={"paused": True}, cookies=cookie
    )
