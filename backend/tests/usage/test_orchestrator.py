...
def test_dedup_preserves_unique_ids():
    ...
    p1 = uuid4()
    p2 = uuid4()
    citation = [{"source_patent_id": p1, "evidence_tier": "medium", "source_type": "forward_citation"}]
    similarity = [{"source_patent_id": p2, "evidence_tier": "strong", "source_type": "similar_newer_patent"}]
    result = dedup_evidence(citation, similarity)
    assert len(result) == 2
