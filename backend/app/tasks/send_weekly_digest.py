...
def fan_out_weekly_digests(self) -> dict:
    """Fan out weekly digests to all subscribed users."""
...
async def _fan_out_async() -> dict:
    ...
    async with async_session_maker() as session:
        # ── fetch all weekly_digest subscribers ──
        ...
        for user_id, sub_list in user_subs.items():
            try:
                ...
                # Fetch matches for each subscribed theme (last 7 days).
                topics_data = []
                matches_data = []
                has_any = False
                ...
                for sub in sub_list:
                    ...
                    for m in match_rows:
                        # Fetch the patent for each match (we need title, assignee, cpc).
                ...
                if not has_any:
                    continue  # skip user with zero matches
                ...
                if result.get("status") in ("sent", "dev", "dry_run"):
                    ...
                    # Update last_delivered_at on each subscription.
                    for sub in sub_list:
