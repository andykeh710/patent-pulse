...
from app.ingestion.uspto_client import USPTOClient, get_last_thursday
...
@celery_app.task(
    bind=True,
    name="app.tasks.ingest_applications.ingest_weekly_applications",
    max_retries=3,
    default_retry_delay=300,
    autoretry_for=(TransientIngestionError,),
)
def ingest_weekly_applications(self, target_date: str | None = None) -> dict:
    """
    Ingest all new USPTO published applications for a given Thursday.
    ...
    logger.info(f"Starting application ingestion for {pub_date}")
    ...
    for raw in client.fetch_applications_by_date(pub_date):
        ...
        try:
            ...
            logger.error(f"Application ingest failed for {pub_number}: {exc}")

    stats["processed"] = stats["created"] + stats["updated"] + stats["failed"]

    if failed_ids:
        logger.warning(
            f"Application ingest completed with {len(failed_ids)} failures: {failed_ids[:10]}"
        )

    logger.info(
        f"Application ingestion complete for {pub_date}: "
...
@celery_app.task(
    bind=True,
    name="app.tasks.ingest_applications.ingest_applications_range",
    max_retries=1,
)
def ingest_applications_range(self, start_date: str, end_date: str) -> dict:
    """
    Ingest applications for a date range. Useful for backfill.
    ...
    logger.info(f"Starting application range ingestion from {start} to {end}")
    ...
    for raw in client.fetch_applications_range(start, end):
        try:
            data = normalizer.normalize_application(raw)
            score, breakdown = scorer.score_dict(data)
            data["interesting_score"] = score
            ...
            stats["failed"] += 1
            logger.error(f"Application ingest failed: {exc}")

    stats["processed"] = stats["created"] + stats["updated"] + stats["failed"]
    logger.info(f"Application range ingestion complete: {stats}")
