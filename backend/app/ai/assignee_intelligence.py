...
def _classify(a):
    ...
    if any(h in j for h in _UNI):
        return "university"
    if any(h in j for h in _GOV):
        return "gov"
    if any(h in j for h in _MEGA):
        return "megacorp"
    return "sme"
...
def _ps(fs):
    return min(fs / 5.0, 1.0)


def _co(c, hc):
    base = {"university": 0.4, "sme": 0.75, "gov": 0.3, "megacorp": 0.9, "unknown": 0.5}.get(c, 0.5)
    return min(base + (0.1 if hc else 0.0), 1.0)
...
def extract_features(p):
    ...
    return AF(
        assignees=list(p.assignees or []),
        assignee_class=c,
        family_size=len(p.family_members or []),
        has_claims=bool(p.claims_text),
        has_abstract=bool(p.abstract),
        industries=list(t.get("industries") or []),
        technology_method=list(t.get("technology_method") or []),
    )
...
def compute_intelligence(f, w=None):
    ...
    wt = 0.0
    funcs = {
        "assignee_type": lambda: _ts(f.assignee_class),
        "portfolio_signals": lambda: _ps(f.family_size),
        "commercial_orientation": lambda: _co(f.assignee_class, f.has_claims),
        "licensing_potential": lambda: _lp(f.assignee_class, f.family_size),
    }
    ...
    if wt > 0:
        total = total / wt
    return {
        "assignee_intelligence_score": round(100.0 * max(0.0, min(total, 1.0)), 2),
...
async def generate_assignee_intelligence(session, patent, *, run_id=None, weights=None):
    f = extract_features(patent)
    w = weights or DEFAULT_WEIGHTS
    rules_hash = hash_rules(RULES_ID, RULES_VERSION, w)
    intel = compute_intelligence(f, w)
    req = RulesArtifactRequest(
        artifact_type="assignee_intelligence",
        rules_id=RULES_ID,
        rules_version=RULES_VERSION,
        rules_hash=rules_hash,
        input_payload=f.as_dict(),
        content_json=intel,
