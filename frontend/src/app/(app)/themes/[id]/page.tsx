    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {theme.cpc_prefixes?.map((c) => (
          <span key={c} className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs">
            CPC: {c}
          </span>
        ))}
        {theme.keywords?.map((k) => (
          <span key={k} className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">
            {k}
          </span>
        ))}
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-3">Recent Matching Patents</h2>
