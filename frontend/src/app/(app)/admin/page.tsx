...
export default function AdminPage() {
  ...
  const { data: usersData, error: usersError, mutate: mutateUsers } = useSWR(
    isAuthenticated && tab === "users" ? ["/api/v1/admin/users"] : null,
    ([url]) => fetch(url, { credentials: "include" }).then((r) => { if (!r.ok) throw r.status; return r.json(); })
  );
  const { data: exports } = useSWR(
    isAuthenticated && tab === "exports" ? ["/api/v1/admin/exports"] : null,
    ([url]) => fetch(url, { credentials: "include" }).then((r) => { if (!r.ok) throw r.status; return r.json(); })
  );
  ...
  const handleTierOverride = async () => {
    ...
    const res = await fetch(`/api/v1/admin/users/${selectedUser.id}/tier`, {
      method: "POST",
      credentials: "include",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      {usersError === 403 ? (
        <div className="p-8 text-red-600">Not authorized. Admin access required.</div>
      ) : (
        <>
